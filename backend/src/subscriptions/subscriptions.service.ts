...
export class SubscriptionsService {
  ...
  async findPlans() {
    ...
    const countMap = counts.reduce((acc: any, c) => {
  ...
  async initPaytrPayment(
    ...
    if (
      !paytrCfg.paytrIsActive ||
      !paytrCfg.paytrMerchantId ||
      !paytrCfg.paytrMerchantKey ||
      !paytrCfg.paytrMerchantSalt
    ) {
      throw new BadRequestException('PayTR sistem ayarları eksik veya pasif');
    }
    ...
    if (!userName) {
      throw new BadRequestException('İsim zorunludur');
    }
    if (!userPhone) {
      throw new BadRequestException('Telefon zorunludur');
    }
    if (!userAddress) {
      throw new BadRequestException('Adres zorunludur');
    }

    const basket = Buffer.from(
      JSON.stringify([[`${plan.name} (${period})`, String(paytrAmount), 1]]),
    ).toString('base64');
    ...
    const hashStr =
      String(paytrCfg.paytrMerchantId) +
      String(userIp || '') +
      merchantOid +
      email +
      String(paytrAmount) +
    ...
    const paytrToken = createHmac('sha256', String(paytrCfg.paytrMerchantKey))
    ...
    try {
      ...
      if (res.data?.status !== 'success' || !res.data?.token) {
        ...
        throw new BadRequestException(`PayTR token alınamadı: ${reason}`);
      }
      ...
      throw e instanceof BadRequestException
        ? e
        : new BadRequestException(`PayTR token alınamadı: ${msg}`);
    }
  }

  async handlePaytrCallback(payload: any) {
    ...
    const expected = createHmac('sha256', String(paytrCfg.paytrMerchantKey))
      .update(hashStr)
      .digest('base64');
  ...
  async getPaytrInstallments(bin: string, _amount: number) {
    void _amount;
    ...
    try {
      ...
      const paytrTokenBin = createHmac(
        'sha256',
        String(paytrCfg.paytrMerchantKey),
      )
        .update(hashStrBin)
        .digest('base64');
      ...
      const paytrTokenRates = createHmac(
        'sha256',
        String(paytrCfg.paytrMerchantKey),
      )
        .update(hashStrRates)
      ...
      const brandRates =
        brand && rates ? rates[brand] || rates[brand?.toLowerCase?.()] : null;
      let options: number[] = [];
      if (brand === 'none' || !brandRates) {
        options = Array.from(
          { length: Math.min(maxInst, 12) },
          (_, i) => i + 1,
        );
      } else {
        const brandRatesObj =
          brandRates && typeof brandRates === 'object' ? brandRates : {};
