...
export class StorageService {
  ...
  async getFolderContents(folderId: string | null, user: any) {
    ...
    if (isClient && !folderId) {
      ...
      if (customerId) {
        ...
        if (myFolder) {
          ...
          const newFolder = await this.ensureCustomerFolder(
            customerId,
            tenantId,
          );
    ...
    if (folderId) {
      ...
      if (isClient) {
        ...
        if (currentFolder.customerId === customerId) {
          ...
          // Check ancestry
          const isDescendant = await this.isFolderDescendantOfCustomer(
            folderId,
            customerId,
          );
          if (!isDescendant) {
    ...
    if (isClient) {
      fileWhere.OR = [{ isPublic: true }, { uploaderId: user.id }];
    }
  ...
  private async isFolderDescendantOfCustomer(
    folderId: string,
    customerId: string,
  ): Promise<boolean> {
  ...
  async uploadFile(
      ...
      if (folder.customerId === user.customerId) {
        ...
        if (!isDescendant) {
          throw new BadRequestException(
            'Bu klasöre dosya yükleme yetkiniz yok',
          );
        }
      }
    }
    ...
    if (tenant.maxStorage) {
      ...
      if (plan && plan.maxStorage) {
        ...
        if (planCode === 'PRO')
          maxStorageBytes = BigInt(100) * BigInt(1024 * 1024 * 1024); // 100GB
        else if (planCode === 'ENTERPRISE')
          maxStorageBytes = BigInt(500) * BigInt(1024 * 1024 * 1024); // 500GB
  ...
  async deleteFolder(folderId: string, user: any) {
    ...
    if (user.role === 'CLIENT') {
      ...
      const isMyTree = await this.isFolderDescendantOfCustomer(
        folderId,
        user.customerId,
      );
      if (!isMyTree && folder.customerId !== user.customerId) {
        // Allow if it is the root customer folder? No, system check handles that.
    ...
    if (folder.files.length > 0 || folder.children.length > 0) {
      throw new BadRequestException(
        'Klasör boş değil. Lütfen önce içeriğini silin.',
      );
    }
  ...
  async moveFile(fileId: string, targetFolderId: string | null, user: any) {
    ...
    if (user.role === 'CLIENT') {
      ...
      // Target Check
      if (targetFolderId) {
        const isTargetDescendant = await this.isFolderDescendantOfCustomer(
          targetFolderId,
          user.customerId,
        );
        // Also check if target is the root customer folder
        const targetFolder = await this.prisma.storageFolder.findUnique({
          where: { id: targetFolderId },
        });
        if (
          targetFolder?.customerId !== user.customerId &&
          !isTargetDescendant
        ) {
