...
async function main() {
  ...
  // Clean up existing Instagram related data
  ...
  for (const user of instagramUsers) {
    ...
    // Messages will be deleted via cascade or we keep them, let's keep them attached to user for now or delete if needed.
    // For clean seed, let's delete messages too.
  ...
  // Also clean up rooms that might have been left over
  ...
  const postRoom = await prisma.chatRoom.create({
    data: {
      ...
      type: 'CHANNEL', // Post comments act like channels
      platform: 'INSTAGRAM',
